...
export default {
	...
	bind: function() {
		...
		$(document).on('change', this.target, function() {
		...
		$(document).on('reset', function(e) {
			...
			if (formReset.is('form')) {
				formReset
					...
					.removeClass('valid')
					.removeClass('invalid')
				formReset.find(this.target).each(function(e) {
		...
		document.addEventListener(
			...
			e => {
				...
				if ($inputElement.is(vm.target)) {
					...
					this.validate_field($inputElement)
				}
			},
			true
		)
		...
		$(text_area_selector).each(function() {
			...
			vm.textareaAutoResize($textarea)
		})

		$(document).on('keyup', text_area_selector, function() {
			vm.textareaAutoResize($(this))
		})
	...
	updateTextFields: function() {
		setTimeout(() => {
			$(this.target).each(function(index, element) {
				...
				// console.log($inputElement[0].value.length === 0)
				if (
